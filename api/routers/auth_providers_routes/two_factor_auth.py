...
from operations.mongo_operations.users_crud import check_apikey_exists
from operations.mongo_operations.end_users_crud import get_end_user_by_identifier, create_end_user, update_end_user
...
async def validate_client_credentials(client_id: str, client_secret: str):
    config = await check_apikey_exists(client_id)
    if not config:
    ...
    from operations.mongo_operations.users_crud import get_all_users
...
async def setup_2fa(inp: Setup2FASchema):
    config = await validate_client_credentials(inp.client_id, inp.client_secret)
    # Retrieve brand name (issuer) from project config, default to "DAuth"
    brand_name = config.get("ui", {}).get("brand_name", "DAuth")
    ...
    end_user = await get_end_user_by_identifier(inp.client_id, user_identifier)
    if not end_user:
        end_user = EndUser(
            id=user_identifier,
        ...
        await create_end_user(inp.client_id, end_user)
    ...
    if not totp_secret:
        ...
        await update_end_user(inp.client_id, end_user)
...
async def setup_verify_2fa(inp: VerifySetup2FASchema):
    await validate_client_credentials(inp.client_id, inp.client_secret)
    user_identifier = get_user_identifier(inp)
